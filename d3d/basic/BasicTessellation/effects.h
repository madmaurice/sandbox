...
#ifndef _INCGUARD_EFFECTS_H
#define _INCGUARD_EFFECTS_H
...
{
public:
	BlurEffect(ID3D11Device* device, const std::string& filename);
	~BlurEffect();
...
{
public:
	TessellationEffect(ID3D11Device* device, const std::string& filename);
	~TessellationEffect();
...
public:
	...
	static std::unique_ptr<BasicEffect> BasicFX;
    static std::unique_ptr<BlurEffect> BlurFX;
    static std::unique_ptr<TessellationEffect> TessellationFX;
};
