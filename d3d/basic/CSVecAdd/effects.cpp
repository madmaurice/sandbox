...
VecAddEffect::VecAddEffect(ID3D11Device* device, const std::string& filename)
	: Effect(device, filename)
{
	VecAddTech  = m_fx->GetTechniqueByName("VecAdd");

	InputA = m_fx->GetVariableByName("gInputA")->AsShaderResource();
	InputB = m_fx->GetVariableByName("gInputB")->AsShaderResource();
	Output = m_fx->GetVariableByName("gOutput")->AsUnorderedAccessView();
}

VecAddEffect::~VecAddEffect() { }

std::unique_ptr<VecAddEffect> Effects::VecAddFX = nullptr;


void Effects::InitAll(ID3D11Device* device)
{
    BasicFX.reset(new BasicEffect(device, "FX/Basic.fxo"));
    VecAddFX.reset(new VecAddEffect(device, "FX/VecAdd.fxo"));
}

void Effects::DestroyAll()
{
    BasicFX.reset(nullptr);
    VecAddFX.reset(nullptr);
}