...
cbuffer cbPerObject
{
	...
	float4x4 gTexTransform; // Allow transformation on the texture coordinate (scaling, rotation, translation...)
	Material gMaterial;
}; 
...
float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure) : SV_Target
{
	...
	{
		// Sample texture.
        // This returns the interpolated color from the texture map at the specified (u, v)
        // point using the filtering methods specified by the SamplerState object.
	...
	{  
		...
		// Modulate with late add (modulate texture color with ambient + diffuse)
