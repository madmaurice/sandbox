...
// Tree sprite effect
TreeSpriteEffect::TreeSpriteEffect(ID3D11Device* device, const std::string& filename)
	: Effect(device, filename)
{
	Light3Tech                = m_fx->GetTechniqueByName("Light3");
	Light3TexAlphaClipTech    = m_fx->GetTechniqueByName("Light3TexAlphaClip");
...
std::unique_ptr<TreeSpriteEffect> Effects::TreeSpriteFX = nullptr;


void Effects::InitAll(ID3D11Device* device)
{
    BasicFX.reset(new BasicEffect(device, "FX/Basic.fxo"));
    TreeSpriteFX.reset(new TreeSpriteEffect(device, "FX/TreeSprite.fxo"));
}

void Effects::DestroyAll()
{
    BasicFX.reset(nullptr);
    TreeSpriteFX.reset(nullptr);
}